...
## synchronized的作用
**`synchronized` 可以同时保证它所保护代码块的原子性、可见性和有序性**。但这三个保证都基于“同一个锁”且“代码在同步块内”这个前提。下面逐一解释。
...
**保证方式**：互斥访问。  
同一时刻只允许一个线程执行被 `synchronized` 保护的代码块，因此该块内的**复合操作**（如“读取-修改-写入”）不会被其它线程打断，对外表现为一个不可分割的整体。

⚠️ 注意：原子性仅针对**同一个锁**保护的代码块。如果两个方法用的不是同一把锁，线程依然可以同时执行它们。

```java
synchronized(lock) {
    count++;   // 读-改-写整体原子，不会被切换
}
```
...
**保证方式**：JMM 的 **happens-before** 规则。  
- 一个线程在**释放锁之前**对共享变量所做的所有修改，对于**随后获取同一个锁**的线程是**完全可见的**。  
- 简单理解：释放锁时把工作内存中的修改刷新到主存，获取锁时重新从主存读取。

因此，只要能正确使用同一个锁保护读写操作，**就不需要额外的 `volatile`** 来保证可见性。

```java
// 线程A
synchronized(lock) {
    data = 5;   // 修改
} // 释放锁时，data 的修改对其他线程可见
...
**保证方式**：禁止临界区内的指令“逃逸”到临界区外。  
- Java 内存模型允许编译器和处理器在**不改变单线程执行结果**的前提下，对指令进行重排序。  
- 但 `synchronized` 规定：  
  - **进入锁前**的代码不能重排到临界区**内**。  
  - **临界区内**的代码不能重排到**释放锁之后**。  
- 这保证了临界区内的代码总是在“获取锁→执行→释放锁”的严格框架内执行。

⚠️ 需要注意的是，**临界区内部**的无依赖指令仍然可能被重排序（比如两个无关系的变量赋值先后调换），但这种重排不会影响单线程的语义，也不会越过锁边界。因此对于正确同步的程序，表现为顺序一致性。

**但 `synchronized` 不能解决“创建对象时指令重排”的问题**（这需要 `volatile`），例如经典的单例双检锁（DCL）中，必须将单例引用声明为 `volatile`，因为 `instance = new Singleton()` 可能被重排为“先分配内存、引用赋值，再初始化对象”，另一个线程在同步块外看到非空引用，但拿到未初始化完成的对象。

---
### 总结

| 特性  | synchronized 能否保证？ | 条件/局限                            |
| --- | ------------------ | -------------------------------- |
| 原子性 | ✅ 保证               | 必须用同一把锁保护所有相关操作                  |
| 可见性 | ✅ 保证               | 锁释放 happens-before 后续加锁          |
| 有序性 | ✅ 保证               | 不能防止临界区外部的乱序（如 DCL 要配合 volatile） |
所以，synchronized 是一种粗粒度但完整的同步机制，只要正确使用同一把锁，它就可以一次性解决三大并发问题。这也是为什么它被作为 Java 最基础的同步原语。
