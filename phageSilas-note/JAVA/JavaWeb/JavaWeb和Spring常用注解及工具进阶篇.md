...
---
## 常用配置
### 1. `propagation`（传播行为）- 决定事务怎样“传染”

这是面试最爱考，也是复杂业务中最常用的配置。它决定了**当一个带有事务的方法，调用另一个带有事务的方法时，这两个事务该怎么相处**。

- **`Propagation.REQUIRED`（默认值）**：
    
    - **逻辑**：如果当前没事务，就建个新的；如果当前有事务，就加入进去。
        
    - **大白话**：大家都在同一条船上，同生共死。只要有一个环节报错，所有人一起回滚。
        
- **`Propagation.REQUIRES_NEW`**：
    
    - **逻辑**：不管当前有没有事务，我都自己新建一个独立的事务。如果外层有事务，就把外层挂起（暂停）。
        
    - **大白话**：各人自扫门前雪。**最典型的场景是“记录操作日志”**：不管核心业务成功还是失败回滚了，日志必须独立提交，写进数据库里。
        
- **`Propagation.NESTED`**：
    
    - **逻辑**：如果当前有事务，就在当前事务里嵌套一个“子事务”（利用数据库的 Savepoint 机制）。
        
    - **大白话**：儿子犯错（子事务异常），只惩罚儿子（子事务回滚）；老子犯错（主事务异常），全家连坐（全部回滚）。
        

### 2. `isolation`（隔离级别）- 解决并发读写问题

当多个用户同时操作同一批数据时，隔离级别决定了他们互相之间能看到什么样的数据。

- **`Isolation.DEFAULT`（默认值）**：完全听从底层数据库的默认配置（MySQL 默认是 `REPEATABLE_READ`，Oracle 默认是 `READ_COMMITTED`）。**99% 的情况下用默认的就行，不要随便改。**
    
- **`Isolation.READ_COMMITTED`（读已提交）**：只能读取到别人已经提交的数据。能防止**脏读**。
    
- **`Isolation.REPEATABLE_READ`（可重复读）**：在同一个事务里，多次读取同一条数据，结果总是一样的（即便别人中途修改并提交了）。能防止**不可重复读**。
    
- _(还有 `READ_UNCOMMITTED` 和 `SERIALIZABLE`，但在实际开发中极少使用，前者太不安全，后者性能太差)_。
    

### 3. `rollbackFor` 与 `noRollbackFor`（回滚规则）- 决定遇到什么错才回滚

- **`rollbackFor`**：指定触发回滚的异常类型。
    
    - **⚠️ 强烈建议配置**：`@Transactional(rollbackFor = Exception.class)`。因为 Spring 默认只抓 `RuntimeException`（运行时异常）和 `Error`，如果你抛出了个普通的 `Exception`（比如文件读写异常、自定义的受检异常），它是不会回滚的，这往往会导致严重的脏数据。
        
- **`noRollbackFor`**：指定遇到某些特定异常时**不回滚**，继续提交。
    
    - **场景**：比如某些不太重要的第三方接口调用失败抛了错，但你希望主业务依然提交成功，就可以把那个特定的异常加在这里。
...
- **作用**：如果你这个方法里**全都是 `SELECT` 查询**，没有任何 `INSERT/UPDATE/DELETE`，加上它非常有好处。
    
- **好处**：
    
    1. Spring 和数据库端会做底层优化，查询速度更快。
        
    2. 防呆机制：如果你在这个方法里手抖写了个 `UPDATE`，运行时会直接报错，防止误改数据。
        

### 5. `timeout`（超时时间）

- **用法**：`@Transactional(timeout = 30)` （单位：秒）
    
- **作用**：限定这个事务最多能执行多久。如果超过设定时间还没执行完（比如遇到死锁，或者一条 SQL 查了太久），系统就会强行中断并回滚这个事务。
    
- **默认值**：`-1`，也就是没有超时限制，完全取决于底层数据库设置。在高并发系统中，合理设置超时可以防止数据库长连接被占满拖垮整个系统。

## 例:
``` java
// 这是一个非常严谨的核心业务配置模板（例如转账、下订单）
@Transactional(
