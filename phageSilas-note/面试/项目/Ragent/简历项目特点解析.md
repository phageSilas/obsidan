...
```
维度1 (SpEL key = getUserId()):
  用户A 点击发送 "北京天气"     → 锁: key:user_A → ✅ 可执行
  用户A 又点击发送 "上海天气"   → 锁: key:user_A → ❌ 拦截（同一把锁！）
  用户B 点击发送 "北京天气"     → 锁: key:user_B → ✅ 可执行（不同用户不同锁）

维度2 (默认 path + userId + md5):
  用户A 点击发送 "北京天气"     → 锁: path:/chat:userId:A:md5:aaa → ✅ 可执行
  用户A 又点击发送 "上海天气"   → 锁: path:/chat:userId:A:md5:bbb → ✅ 可执行（参数不同！）
  用户A 再点发送 "北京天气"     → 锁: path:/chat:userId:A:md5:aaa → ❌ 拦截（参数相同）
```
